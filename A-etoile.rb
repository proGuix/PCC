...
=end
  ...
  def test
    ...
    tGraph=initGraph(hVectBord,obstacle(tSurface))
    ...
    while !(tOpenSet.empty?) do
      ...
      if (hCurrent[:x]==hFinish[:x])&&(hCurrent[:y]==hFinish[:y])
        p "Final path : " + reconstructPath(hFather, hCurrent).to_s
        return
      end
      tClosedSet<< hCurrent
      ...
      tNeigh.each do |hNeigh|
        if tClosedSet.select{|hPt|hPt[:x]==hNeigh[:x]&&hPt[:y]==hNeigh[:y]}.empty?
          ...
          if hNGraph==nil||(hNGraph!=nil&&hNGraph[:cost]!=INF)
